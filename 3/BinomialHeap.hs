...
isolate :: a -> [a] -> [(a, [a])]
isolate placeHolder [] = []
isolate placeHolder list = pairs ++ lastPair
    where pairs = [(list !! i, getList $ mapAccumL removeAt i indexedList) | i <- [0.. lastIdx - 1]]
          lastPair = [(list !! lastIdx, removeLast list)]
          lastIdx = length list - 1
          indexedList = zip [0.. lastIdx] list
          removeAt = \i (index, x) -> if i == index then (i, (index, placeHolder)) else (i, (index, x))
          getList = snd . unzip . snd
...
removeMin heap@(BinomialHeap s ts) = if isEmptyHeap heap
                                        then heap
                                        else BinomialHeap (s - 1) (mergeTrees orphans replacedMin)
    where pair = head $ filter eqMin $ isolate EmptyTree ts
          eqMin = \(t, trees) -> t == min
          orphans = reverse . children . fst $ pair
          replacedMin = snd pair
          min = minimumBy (compare `on` prio) [t | t <- ts, t /= EmptyTree]
          isEmptyHeap heap = s == 0 && null ts
...
instance (Show p, Show k) => Show (BinomialTree p k) where
    ...
    show tree = show' 0 tree
        where indentedShow lvl tree@(Node p k _) = concat (replicate lvl "  ") ++ show p ++ " (" ++ show k ++ ")"
              show' lvl tree@(Node p k []) = indentedShow lvl tree
              show' lvl tree@(Node p k chs) = indentedShow lvl tree ++ "\n" ++ intercalate "\n" [show' (lvl + 1) child | child <- chs]
...
instance (Show p, Show k) => Show (BinomialHeap p k) where
    show heap@(BinomialHeap s ts) = intercalate "\n" [show t | t <- ts]
...
instance Foldable (BinomialTree p) where
    ...
    foldr f acc (Node p k chs) = f k $ composedFuncs acc
        where partialFuncs = [\acc' -> foldr f acc' ch | ch <- chs]
              composedFuncs = foldr (.) id partialFuncs
